...
def forum_thread_created_handler(signal, sender, thread, metadata, **kwargs):  # pylint: disable=unused-argument
    """
    Handler for forum thread created event.
    """
    notify_users(thread, object_type="thread")


@receiver(FORUM_THREAD_RESPONSE_CREATED)
def forum_response_created_handler(signal, sender, thread, metadata, **kwargs):  # pylint: disable=unused-argument
    """
    Handler for forum response created event.
    """
    notify_users(thread, object_type="response")


@receiver(FORUM_RESPONSE_COMMENT_CREATED)
def forum_comment_created_handler(signal, sender, thread, metadata, **kwargs):  # pylint: disable=unused-argument
    """
    Handler for forum comment created event.
    """
    notify_users(thread, object_type="comment")
...
def notify_users(thread, object_type):
    ...
    elif object_type in ("response", "comment"):
